...
<div class="card">
    ...
    <form class="form-horizontal" action="{{route('order.store')}}" method="post">
        ...
        <div class="card-body">
            ...
            <div class="form-group row">
                    <label class="col-sm-2 text-right control-label col-form-label">Food Item</label>
                    <div class="col-md-3">

                        <select class="select2 form-control custom-select" style="width: 100%; height:36px;" name="food_item_id" id="food">
                            <option>Select Foode item</option>
                          @foreach($food as $k=>$f)
                            <option value="{{$f->id}}">{{$f->item}}</option>
                          @endforeach
                        </select>
                    </div>
            </div>
